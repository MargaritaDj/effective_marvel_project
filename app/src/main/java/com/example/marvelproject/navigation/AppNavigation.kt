...
@Composable
fun AppNavigation(listHeroes: List<Hero>, error: Boolean, idPushHero: String, context: Context){
    ...
    NavHost(navController = navController, startDestination = Routes.ListHeroes.route + "/{id}"){
        ...
        composable(Routes.ListHeroes.route + "/{id}"){ backStackEntry ->
            ...
            ListAllHeroes(listHeroes, navController, id, error, idPushHero, context)
        }
